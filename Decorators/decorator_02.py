...
def logger(name_file):
    ...
    Для использования декоратора разместите его над нужной функцией @logger_name('имя log файла').
    Имя log файла указывать без расширения.
    :param name_file: указать имя файла без расширения.
    :return:
    '''
    def in_function(income_function):
        def log_function(*args, **kwargs):
            ...
            with open(name_file+'.txt', mode='a', encoding='UTF-8') as file:
                file.write(text)
            return result
        return log_function
    return in_function
